...
export function ManageSingle({
  ...
  const stakingToken = wrappedCurrency(currency ?? undefined, chainId) ?? UNDEFINED[chainId ? chainId : ChainId.DOGECHAIN]
  const rewardToken = wrappedCurrency(rewardCurrency ?? undefined, chainId) ?? UNDEFINED[chainId ? chainId : ChainId.DOGECHAIN]
  const [, stakingTokenPair] = usePair(stakingToken, UNDEFINED[chainId ? chainId : ChainId.DOGECHAIN])
